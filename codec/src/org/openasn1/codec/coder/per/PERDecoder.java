...
import static org.openasn1.codec.util.ASN1Constants.BITPATTERN_00001111;
import static org.openasn1.codec.util.ASN1Constants.BITPATTERN_00111111;
import static org.openasn1.codec.util.ASN1Constants.BITPATTERN_01000000;
...
import org.openasn1.codec.coder.exceptions.DecoderException;
...
import org.openasn1.codec.coder.typecoder.TypeDecoder;
import org.openasn1.codec.coder.typecoder.TypeDecoderComparator;
...
import org.openasn1.codec.util.bit.BasicBitInputStream;
import org.openasn1.codec.util.bit.BitField;
import org.openasn1.codec.util.bit.BitInputStream;
...
abstract public class PERDecoder implements ASN1Decoder {
	 ...
	 * 
	 * @see org.openasn1.codec.coder.ASN1Decoder#decodeNull(org.openasn1.codec.coder.typecoder.TypeInformation)
	 */
     ...
     * @see "X.691-0207 10.6"
     */
	protected long decodeNormallySmallNonNegativeWholeNumber() throws IOException {
		/**
     ...
     * @see "X.691-0207 10.7"
     */
	protected long decodeSemiConstrainedWholeNumber(long constrainedRootMinimum) throws IOException {
		/**
         * Note: since we assume java integer encoding on a maximum of 32 bits
         * we read the encoded value into an int. If one needs to read larger
         * values, one can extend the functionality to return Java-"BigInt"s
		...
		return offsetValue + constrainedRootMinimum;
	}
	...
	public long decodeInteger(TypeInformation typeInformation) throws IOException {
		if (typeInformation.hasConstraint()) {
	...
	private long decodeConstrainedInteger(TypeInformation typeInformation) throws IOException {
		Constraint constraint = typeInformation.getConstraint();
		...
		long constrainedRootMinimum = 0;
		long constrainedRootMaximum = 0;
	...
	public void decodeSequence(List<TypeDecoder> componentDecoderList, TypeInformation typeInformation) throws IOException {
         ...
         */
		long additionBitmapLength = decodeNormallySmallNonNegativeWholeNumber() + 1;
         ...
         * @see "X.680-0207 18.7"
         */
		boolean shouldDecodeAdditionComponent[] = new boolean[(int) additionBitmapLength];
	...
	public <T> List<T> decodeConstrainedSequenceOf(TypeDecoder<T> decoderPrototype, TypeInformation typeInformation) throws IOException {
		...
		long components = constrainedRootMinimum;
		...
		ArrayList<T> decodedComponentList = new ArrayList<T>((int) components);
	...
	public synchronized void decodeChoice(List<TypeDecoder> componentDecoderList, TypeInformation typeInformation) throws IOException {
		...
		if (!extensionAdditionHasBeenEncoded) {
			if (shouldDecodeIndex) {
				...
				selectedAlternativeTypeDecoder = sortedList.get((int) encodingIndex);
			} else {
				/**
			...
			selectedAlternativeTypeDecoder = extensionAdditionComponentDecoderList.get((int) encodingIndex);
		}
     ...
     */
	protected int decodeLengthField(long lowerBound) throws IOException {
	...
	protected byte[] decodeConstrainedOctetString(TypeInformation typeInformation) throws IOException {
		...
		if (isWithinTheExtensionRoot && hasConstrainedRootMaximum) {
			...
			if ((lowerBound == upperBound) && (upperBound <= 2)) {
				byte[] data = new byte[(int) upperBound];
     ...
     */
	protected long decodeLengthField(Long long1, Long long2) throws IOException {
		if ((long1 == long2) && (long1 < SIZE_64K)) {
			/**
		...
		if (long2 < SIZE_64K) {
			return decodeConstrainedWholeNumber(long1, long2);
			/**
		...
		return decodeLengthField(long1);
	}
     ...
     */
	protected long decodeConstrainedWholeNumber(long constrainedRootMinimum, long constrainedRootMaximum) throws IOException {
		int range = (int) (constrainedRootMaximum - constrainedRootMinimum + 1);
