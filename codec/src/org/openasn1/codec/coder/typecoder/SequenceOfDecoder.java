...
public class SequenceOfDecoder<T> extends TypeDecoder<List<T>> {
	...
	public SequenceOfDecoder(TypeDecoder<T> decoderPrototype, TypeInformation typeInformation) {
	...
	public List<T> decode(ASN1Decoder decoder) throws IOException {
		return decoder.decodeSequenceOf(getDecoderPrototype(), getTypeInformation());
	}
}