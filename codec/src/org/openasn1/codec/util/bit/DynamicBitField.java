 ...
 * 
 */
package org.openasn1.codec.util.bit;
...
public class DynamicBitField extends BitFieldImpl {
	private static int FIELD_SIZE = 32; //32 chnaged to 64
	...
	public void setValue(int startBit, int endBit, int value) {
		...
		throw new IndexOutOfBoundsException(" DynamicBitField :setValue not implemented");
	}

	public void setValue(int startBit, int endBit, long value) {
		//System.out.print("DynamicBitField: setValue : "+value + " s "+ startBit+ " e "+endBit + "\n");
		if (startBit < 0) {
			throw new IndexOutOfBoundsException("The bit indices may not be negative");
		}
		...
		if ((endBit - startBit) > 63) {
			throw new RuntimeException("Range exceeds the scale of an integer-now changed to long");
		}
	...
	public void setBit(int index, int value) {
		//System.out.print("DynamicBitField: setBit : "+ value + " index "+ index+ "\n");
	...
	public void appendValue(long value, int bits) {
		//System.out.print("DynamicBitField: appendValue : "+value + " "+bits +"\n");
		if (bits < 0) {
			throw new IndexOutOfBoundsException("Only a positive amount of bits may be appended");
		}

		if (bits > 64) {
			throw new IndexOutOfBoundsException("An integer only has 32 bits- now changed to long 64 bits");
		}
