...
import org.openasn1.compiler.astutils.visitors.GetDefaultTaggingModeVisitor;
 ...
 *
 */
public class AutomaticTagDecider implements ASN1ASTModifier {
    ...
    public ASN1ASTStorage modifyAST(ASN1ASTStorage storage) {
        for (Node ast : storage.getAsn1AstList()) {
            ...
            ast.accept(defvis);
            if (defvis.getDefinedTaggingMode() == TaggingModeEnum.AUTOMATIC) {
                AutomaticTaggingDecidingVisitor vis = new AutomaticTaggingDecidingVisitor(
